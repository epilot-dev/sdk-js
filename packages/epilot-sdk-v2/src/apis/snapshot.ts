...
const resolve = (): Client => {
  if (!_instance) {
    ...
    _instance = createApiClient<Client>({ definition: def, apiName: 'snapshot' });
  }
  return _instance;
};
...
const _handle: ApiHandle<Client> = createApiHandle({
  ...
  createClient: () => createApiClient<Client>({ definition: loadDefinition(), apiName: 'snapshot' }),
  apiName: 'snapshot',
});
 ...
 * `snapshot.someOperation(...)` calls forwarded to lazy singleton
 */
export const snapshot = _handle;

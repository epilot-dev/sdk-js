...
const loadDefinition = (): Document => {
  const mod = require('../definitions/user-runtime.json')
...
const resolve = (): Client => {
  if (!_instance) {
    ...
    _instance = createApiClient<Client>({ definition: def, apiName: 'user' })
  }
  return _instance
}
...
const _handle: ApiHandle<Client> = createApiHandle({
  ...
  createClient: () => createApiClient<Client>({ definition: loadDefinition(), apiName: 'user' }),
  apiName: 'user',
})

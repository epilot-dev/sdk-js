...
const loadDefinition = (): Document => {
  const mod = require('../definitions/access-token-runtime.json');
...
const resolve = (): Client => {
  if (!_instance) {
    ...
    _instance = createApiClient<Client>({ definition: def, apiName: 'accessToken' });
  }
  return _instance;
};
...
const _handle: ApiHandle<Client> = createApiHandle({
  ...
  createClient: () => createApiClient<Client>({ definition: loadDefinition(), apiName: 'accessToken' }),
  apiName: 'accessToken',
});

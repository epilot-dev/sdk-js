...
const loadDefinition = (): Document => {
  const mod = require('../definitions/chat-runtime.json');
...
const resolve = (): Client => {
  if (!_instance) {
    ...
    _instance = createApiClient<Client>({ definition: def, apiName: 'chat' });
  }
  return _instance;
};
...
const _handle: ApiHandle<Client> = createApiHandle({
  ...
  createClient: () => createApiClient<Client>({ definition: loadDefinition(), apiName: 'chat' }),
  apiName: 'chat',
});
 ...
 * `chat.someOperation(...)` calls forwarded to lazy singleton
 */
export const chat = _handle;

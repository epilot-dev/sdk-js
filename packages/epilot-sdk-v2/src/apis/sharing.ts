...
import type { Client } from '../types/sharing'
export type * from '../types/sharing'
export type { OpenAPIClient } from 'openapi-client-axios'

/* eslint-disable @typescript-eslint/no-require-imports */
const loadDefinition = (): Document => {
  const mod = require('../definitions/sharing-runtime.json')
...
const resolve = (): Client => {
  if (!_instance) {
    ...
    _instance = createApiClient<Client>({ definition: def, apiName: 'sharing' })
  }
  return _instance
}
...
const _handle: ApiHandle<Client> = createApiHandle({
  ...
  createClient: () => createApiClient<Client>({ definition: loadDefinition(), apiName: 'sharing' }),
  apiName: 'sharing',
})

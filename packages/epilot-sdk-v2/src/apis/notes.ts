...
const loadDefinition = (): Document => {
  const mod = require('../definitions/notes-runtime.json')
...
const resolve = (): Client => {
  if (!_instance) {
    ...
    _instance = createApiClient<Client>({ definition: def, apiName: 'notes' })
  }
  return _instance
}
...
const _handle: ApiHandle<Client> = createApiHandle({
  ...
  createClient: () => createApiClient<Client>({ definition: loadDefinition(), apiName: 'notes' }),
  apiName: 'notes',
})

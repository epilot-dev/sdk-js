...
const loadDefinition = (): Document => {
  const mod = require('../definitions/calendar-runtime.json');
...
const resolve = (): Client => {
  if (!_instance) {
    ...
    _instance = createApiClient<Client>({ definition: def, apiName: 'calendar' });
  }
  return _instance;
};
...
const _handle: ApiHandle<Client> = createApiHandle({
  ...
  createClient: () => createApiClient<Client>({ definition: loadDefinition(), apiName: 'calendar' }),
  apiName: 'calendar',
});
 ...
 * `calendar.someOperation(...)` calls forwarded to lazy singleton
 */
export const calendar = _handle;

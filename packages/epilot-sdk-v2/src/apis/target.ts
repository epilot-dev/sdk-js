...
import type { Client } from '../types/target'
export type * from '../types/target'
export type { OpenAPIClient } from 'openapi-client-axios'

/* eslint-disable @typescript-eslint/no-require-imports */
const loadDefinition = (): Document => {
  const mod = require('../definitions/target-runtime.json')
...
const resolve = (): Client => {
  if (!_instance) {
    ...
    _instance = createApiClient<Client>({ definition: def, apiName: 'target' })
  }
  return _instance
}
...
const _handle: ApiHandle<Client> = createApiHandle({
  ...
  createClient: () => createApiClient<Client>({ definition: loadDefinition(), apiName: 'target' }),
  apiName: 'target',
})

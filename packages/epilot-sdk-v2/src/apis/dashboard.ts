...
const loadDefinition = (): Document => {
  const mod = require('../definitions/dashboard-runtime.json')
...
const resolve = (): Client => {
  if (!_instance) {
    ...
    _instance = createApiClient<Client>({ definition: def, apiName: 'dashboard' })
  }
  return _instance
}
...
const _handle: ApiHandle<Client> = createApiHandle({
  ...
  createClient: () => createApiClient<Client>({ definition: loadDefinition(), apiName: 'dashboard' }),
  apiName: 'dashboard',
})

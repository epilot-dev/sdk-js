...
declare namespace Components {
  namespace Schemas {
    ...
    export type Role =
      ...
      | UserRole
      | /* A role automatically applied to all users in an organization. */ OrgRole
      | /* A role that can be assigned to users in other organizations for sharing purposes. */ ShareRole
      | /* A role that appears in another organization's role list that can be assigned but not modified by the partner organization. */ PartnerRole
      | /* A role that is applied to end customers and installers using the Portals */ PortalRole;
    /**
     * Format: <organization_id>:<slug>
     * example:
     * 123:owner

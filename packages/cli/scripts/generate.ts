...
import { collectRepeatedFlag } from '../../lib/flag-collector.js';
...
export default defineCommand({
  ...
  run: ({ args, rawArgs }) => {
    ...
    // citty (via node:util.parseArgs) only keeps the last occurrence of a
    // repeated string flag, so args.param silently drops earlier -p flags.
    // Recover all of them directly from rawArgs instead.
    const paramFlags = collectRepeatedFlag(rawArgs ?? [], ['p', 'param']);

    return callApi('${client.kebabName}', {
      ...args,
      param: paramFlags.length > 0 ? paramFlags : args.param,

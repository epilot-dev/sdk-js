...
const sanitizeDescription = (desc: string): string => {
  ...
  return firstParagraph.replace(/^#{1,6}\s+/gm, '').trim().substring(0, 300);
};
...
const extractOperations = (
  ...
  if (!existsSync(specPath))
    return { title: apiName, description: '', serverUrl: '', operations: [] };
  ...
  for (const [path, methods] of Object.entries(
    ...
    for (const method of ['get', 'post', 'put', 'patch', 'delete', 'head', 'options']) {
      ...
      const reqBodySchema = (reqBodyContent?.['application/json'] as Record<string, unknown>)
      ...
      const respSchema = (respContent?.['application/json'] as Record<string, unknown>)?.schema as
        | Schema
        | undefined;
...
const generateApiList = (clients: ClientInfo[]): string => {
  const entries = clients
    ...
    .map((c) => {
      const opIds = c.operations.map((op) => op.operationId);
      return `  { apiName: '${c.apiName}', kebabName: '${c.kebabName}', title: ${JSON.stringify(c.title)}, serverUrl: '${c.serverUrl}', operationCount: ${c.operations.length}, operationIds: ${JSON.stringify(opIds)} }`;
    });
...
const generateClientDoc = (client: ClientInfo): string => {
  ...
  lines.push(
    `- **API Docs:** [https://docs.epilot.io/api/${kebabName}](https://docs.epilot.io/api/${kebabName})`,
  );
  lines.push(``);
  if (description) {
...
const generateOperationDoc = (kebabName: string, op: OperationInfo): string[] => {
  ...
  if (op.requestBodySchema) {
    lines.push(
      `**Request Body**${op.requestBodyRequired ? ' (required)' : ''}`,
      ``,
    );
  }
  ...
  lines.push('```bash');
  {
    ...
    lines.push(
      `epilot ${kebabName} ${op.operationId}${pFlags} --jsonata '${jsonataExpr}'`,
    );
  }
  lines.push('```');
  lines.push(``);
...
const generateDocs = (clients: ClientInfo[]) => {
  ...
  indexLines.push(`epilot profile create staging --server https://entity.staging.sls.epilot.io --token <staging-token>`);
  indexLines.push(``);
...
const updateReadme = (clients: ClientInfo[]): void => {
  ...
  const apiNames = ['auth', 'profile', 'completion', ...validClients.map((c) => c.kebabName)];
  const usageLine = `USAGE epilot [OPTIONS] ${apiNames.join('|')}`;

  const maxNameLen = Math.max(...apiNames.map((n) => n.length));
  const commandLines = [
    `${'auth'.padStart(maxNameLen)}    Manage authentication`,
    `${'profile'.padStart(maxNameLen)}    Manage profiles (server URLs, tokens, environments)`,
    `${'completion'.padStart(maxNameLen)}    Generate shell completion scripts`,
    ...validClients.map((c) => `${c.kebabName.padStart(maxNameLen)}    ${c.title}`),
  ];
  ...
  readme = readme.replace(
    /<!-- api-reference-table -->[\s\S]*?<!-- \/api-reference-table -->/,
    apiTable,
  );

...
- [`validateFile`](#validatefile) — Validate an uploaded file against one or more document validation rules
...
```bash
epilot file validateFile \
  -d '{"file":{"entity_id":"ef7d985c-2385-44f4-9c71-ae06a52264f8"},"rule_ids":["string"]}'
```

Using stdin pipe:

```bash
cat body.json | epilot file validateFile
```

With JSONata filter:

```bash
epilot file validateFile --jsonata 'results[0]'
```
